...
public class MonthlyGoalControllerTest extends RestDocsTest {
	...
	void monthlyGoalAddTest() throws Exception {
		...
		MonthlyGoalUpdateRequest request = new MonthlyGoalUpdateRequest("취뽀하기!", "취업할 때까지 숨 참는다.",
				LocalDate.of(2024, 4, 1), LocalDate.of(2024, 4, 30), 1L, 1L);

		ResultActions perform = mockMvc.perform(post("/api/v1/calendars/{calendarId}/monthly-goals", 1L)
				.headers(authorizationHeader()).contentType(MediaType.APPLICATION_JSON).content(toJson(request)));

		perform.andExpect(status().isCreated());

		perform.andDo(print()).andDo(document("monthly-goal-add", getDocumentRequest(), getDocumentResponse(),
	...
	void monthlyGoalModifyTest() throws Exception {
		...
		MonthlyGoalUpdateRequest request = new MonthlyGoalUpdateRequest("(수정)취뽀하기!", "(수정)취업할 때까지 숨 참는다.",
				LocalDate.of(2024, 5, 1), LocalDate.of(2024, 5, 31), 1L, 1L);

		ResultActions perform = mockMvc
				.perform(put("/api/v1/calendars/{calendarId}/monthly-goals/{monthlyGoalId}", 1L, 1L)
						.headers(authorizationHeader()).contentType(MediaType.APPLICATION_JSON)
						.content(toJson(request)));

		perform.andExpect(status().isNoContent());

		perform.andDo(print()).andDo(document("monthly-goal-modify", getDocumentRequest(), getDocumentResponse(),
				requestHeaders(authorizationDesc())));
	}
	...
	void monthlyGoalModifyTestMonthlyGoalNotFound() throws Exception {
		...
		MonthlyGoalUpdateRequest request = new MonthlyGoalUpdateRequest("(수정)취뽀하기!", "(수정)취업할 때까지 숨 참는다.",
				LocalDate.of(2024, 5, 1), LocalDate.of(2024, 5, 31), 1L, 1L);
		willThrow(new RecordNotFoundException(ErrorCode.MONTHLY_GOAL_NOT_FOUND)).given(monthlyGoalUpdateService)
		...
		perform.andDo(print()).andDo(document("monthly-goal-modify-monthly-goal-not-found", getDocumentResponse()));
	}
	...
	void monthlyGoalListTest() throws Exception {
		...
		ResultActions perform = mockMvc.perform(get("/api/v1/calendars/{calendarId}/monthly-goals", 1L)
		...
		perform.andExpect(status().isOk()).andExpect(jsonPath("$.size()").value(3))
				.andExpect(jsonPath("$.[0].id").value(1)).andExpect(jsonPath("$.[0].title").value("취뽀하기!"))
				.andExpect(jsonPath("$.[0].achieved").value(false)).andExpect(jsonPath("$.[1].id").value(2))
				.andExpect(jsonPath("$.[1].title").value("취뽀하기!")).andExpect(jsonPath("$.[1].achieved").value(true))
				.andExpect(jsonPath("$.[2].id").value(3)).andExpect(jsonPath("$.[2].title").value("취뽀하기!"))
				.andExpect(jsonPath("$.[2].achieved").value(false));

		perform.andDo(print())
				.andDo(document("monthly-goal-list", getDocumentRequest(), getDocumentResponse(),
						requestHeaders(authorizationDesc()),
	...
	void monthlyGoalDetailsTest() throws Exception {
		...
		given(monthlyGoalQueryService.searchByIdAndCheckAuthority(anyLong())).willReturn(monthlyGoal);

		ResultActions perform = mockMvc.perform(
				get("/api/v1/calendars/{calendarId}/monthly-goals/{id}", 1L, 1L).headers(authorizationHeader()));

		perform.andExpect(status().isOk()).andExpect(jsonPath("$.id").value(monthlyGoal.getId()))
		...
		perform.andDo(print()).andDo(document("monthly-goal-details", getDocumentRequest(), getDocumentResponse(),
				requestHeaders(authorizationDesc()), pathParameters(parameterWithName("id").description("조회할 월 목표 id"),
	...
	void monthlyGoalDetailsNotFoundTest() throws Exception {
		...
		perform.andDo(print()).andDo(document("monthly-goal-details-monthly-goal-not-found", getDocumentResponse()));
	}
	...
	void monthlyGoalChangeAchieveStatusTest() throws Exception {
		...
		willDoNothing().given(monthlyGoalUpdateService).changeAchieveStatus(anyLong(), anyBoolean(), any());

		ResultActions perform = mockMvc
				.perform(patch("/api/v1/calendars/{calendarId}/monthly-goals/{id}/achieve", 1L, 1L)
						.headers(authorizationHeader()).queryParam("status", "true"));

		perform.andExpect(status().isNoContent());

		perform.andDo(print())
				.andDo(document("monthly-goal-change-achieve-status", getDocumentRequest(), getDocumentResponse(),
						requestHeaders(authorizationDesc()),
						pathParameters(parameterWithName("id").description("달성 상태를 변경할 월 목표 id"),
								parameterWithName("calendarId").description("캘린더 id")),
	...
	void monthlyGoalChangeAchieveStatusNotFoundTest() throws Exception {
		...
		perform.andDo(print()).andDo(document("monthly-goal-change-achieve-status-not-found", getDocumentResponse()));
	}
	...
	void monthlyGoalRemoveTest() throws Exception {
		...
		ResultActions perform = mockMvc.perform(
				delete("/api/v1/calendars/{calendarId}/monthly-goals/{id}", 1L, 1L).headers(authorizationHeader()));

		perform.andExpect(status().isNoContent());

		perform.andDo(print()).andDo(document("monthly-goal-remove", getDocumentRequest(), getDocumentResponse(),
				requestHeaders(authorizationDesc()), pathParameters(parameterWithName("id").description("삭제할 월 목표 id"),
						parameterWithName("calendarId").description("캘린더 id"))));
	}
}
