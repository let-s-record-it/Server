...
class CalendarColorHexTest {
	...
	void throwInvalidColorHexExceptionIfNotFitColorHex() {
		assertAll(() -> {
			assertThatCode(() -> new CalendarColorHex("11233")).isInstanceOf(InvalidColorHexException.class)
					.hasMessage(ErrorCode.INVALID_CALENDAR_COLOR_HEX.getDescription());
			assertThatCode(() -> new CalendarColorHex("ggg")).isInstanceOf(InvalidColorHexException.class)
					.hasMessage(ErrorCode.INVALID_CALENDAR_COLOR_HEX.getDescription());
			assertThatCode(() -> new CalendarColorHex("11233")).isInstanceOf(InvalidColorHexException.class)
					.hasMessage(ErrorCode.INVALID_CALENDAR_COLOR_HEX.getDescription());
			assertThatCode(() -> new CalendarColorHex("ffda12f")).isInstanceOf(InvalidColorHexException.class)
					.hasMessage(ErrorCode.INVALID_CALENDAR_COLOR_HEX.getDescription());
			assertThatCode(() -> new CalendarColorHex("fb")).isInstanceOf(InvalidColorHexException.class)
					.hasMessage(ErrorCode.INVALID_CALENDAR_COLOR_HEX.getDescription());
		});
	}
}
