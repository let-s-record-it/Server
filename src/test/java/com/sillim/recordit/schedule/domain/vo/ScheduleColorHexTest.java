...
class ScheduleColorHexTest {
	...
	void throwInvalidColorHexExceptionIfNotFitColorHex() {
		assertAll(() -> {
			assertThatCode(() -> new ScheduleColorHex("11233")).isInstanceOf(InvalidColorHexException.class)
					.hasMessage(ErrorCode.INVALID_SCHEDULE_COLOR_HEX.getDescription());
			assertThatCode(() -> new ScheduleColorHex("ggg")).isInstanceOf(InvalidColorHexException.class)
					.hasMessage(ErrorCode.INVALID_SCHEDULE_COLOR_HEX.getDescription());
			assertThatCode(() -> new ScheduleColorHex("11233")).isInstanceOf(InvalidColorHexException.class)
					.hasMessage(ErrorCode.INVALID_SCHEDULE_COLOR_HEX.getDescription());
			assertThatCode(() -> new ScheduleColorHex("ffda12f")).isInstanceOf(InvalidColorHexException.class)
					.hasMessage(ErrorCode.INVALID_SCHEDULE_COLOR_HEX.getDescription());
			assertThatCode(() -> new ScheduleColorHex("fb")).isInstanceOf(InvalidColorHexException.class)
					.hasMessage(ErrorCode.INVALID_SCHEDULE_COLOR_HEX.getDescription());
		});
	}
}
