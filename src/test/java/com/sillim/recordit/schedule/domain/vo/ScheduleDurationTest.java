...
class ScheduleDurationTest {
    ...
    void validScheduleDurationIfStartDateIsBeforeEndDate() {
        ...
        assertAll(() -> {
            assertThat(scheduleDuration1).isEqualTo(ScheduleDuration.createNotAllDay(start, end));
            assertThat(scheduleDuration2).isEqualTo(ScheduleDuration.createNotAllDay(start, end));
            assertThat(allDayScheduleDuration1).isEqualTo(
                    ScheduleDuration.createAllDay(start, end));
            assertThat(allDayScheduleDuration2).isEqualTo(
                    ScheduleDuration.createAllDay(start, end));
            assertThat(allDayScheduleDuration1).isNotEqualTo(
                    ScheduleDuration.createNotAllDay(start, end));
        });
    }
