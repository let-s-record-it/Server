...
class ScheduleServiceTest {
    ...
    void addSchedule() {
        ...
        Schedule schedule = Schedule.builder()
                ...
                .build();
        given(scheduleGroupService.addScheduleGroup(false, calendarId, memberId)).willReturn(
                scheduleGroup);
        ...
        assertAll(() -> {
            ...
            assertThat(schedules.get(0).getTitle()).isEqualTo(new Title("title"));
            assertThat(schedules.get(0).getDescription()).isEqualTo(new Description("description"));
            assertThat(schedules.get(0).getScheduleDuration()).isEqualTo(
                    ScheduleDuration.createNotAllDay(LocalDateTime.of(2024, 1, 1, 0, 0),
    ...
    void addRepeatingSchedules() {
        ...
        RepetitionAddRequest repetitionAddRequest = new RepetitionAddRequest(RepetitionType.DAILY,
                1, repetitionStartDate, repetitionEndDate, null, null, null, null, null);
        ScheduleAddRequest scheduleAddRequest = new ScheduleAddRequest("title", "description",
                false, startDate, endDate, true, repetitionAddRequest, "#aaffbb", "서울역",
                true, 36.0, 127.0, true, LocalDateTime.of(2024, 1, 1, 0, 0), calendarId);
        ScheduleGroup scheduleGroup = ScheduleGroup.builder()
                .isRepeated(false)
                .member(member)
                .calendar(calendar).build();
        RepetitionPattern repetitionPattern = RepetitionPattern.createDaily(1, repetitionStartDate,
        ...
        Schedule schedule = Schedule.builder()
                ...
                .scheduleGroup(scheduleGroup)
                .build();
        given(scheduleGroupService.addScheduleGroup(true, calendarId, memberId)).willReturn(
                scheduleGroup);
        given(scheduleRepository.save(any(Schedule.class))).willReturn(schedule);
        given(repetitionPatternService.addRepetitionPattern(repetitionAddRequest,
                scheduleGroup)).willReturn(repetitionPattern);
