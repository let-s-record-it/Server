...
class CustomFeedCommentRepositoryTest {
	...
	void findPaginatedOrderByCreatedDesc() {
		List<FeedComment> feedComments = IntStream.range(0, 10)
				.mapToObj(i -> em.persist(new FeedComment("content", feed, memberId))).toList();

		Slice<FeedComment> foundFeedComments1 = customFeedCommentRepository
				.findPaginatedOrderByCreatedAtAsc(PageRequest.of(0, 5), feed.getId());

		Slice<FeedComment> foundFeedComments2 = customFeedCommentRepository
				.findPaginatedOrderByCreatedAtAsc(PageRequest.of(3, 3), feed.getId());

		assertAll(() -> {
			assertThat(foundFeedComments1).hasSize(5);
			assertThat(foundFeedComments1.isLast()).isFalse();
			assertThat(foundFeedComments1.getContent().get(0).getId()).isEqualTo(feedComments.get(0).getId());
	...
	void findPaginatedByMemberIdOrderByCreatedDesc() {
		...
		Slice<FeedComment> foundFeedComments1 = customFeedCommentRepository
				.findByMemberIdOrderByCreatedAtAsc(PageRequest.of(0, 5), memberId);

		Slice<FeedComment> foundFeedComments2 = customFeedCommentRepository
				.findByMemberIdOrderByCreatedAtAsc(PageRequest.of(3, 3), memberId);
