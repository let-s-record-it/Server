...
@Slf4j
public class CustomInviteLinkRepositoryImpl extends QuerydslRepositorySupport implements CustomInviteLinkRepository {
	...
	public InviteLink findInfoByInviteCode(String inviteCode) {
		return selectFrom(inviteLink).leftJoin(inviteLink.calendar).fetchJoin()
				// .leftJoin(inviteLink.calendar.member)
				// .fetchJoin()
				.where(inviteLink.inviteCode.eq(inviteCode)).fetchOne();
	}
}
