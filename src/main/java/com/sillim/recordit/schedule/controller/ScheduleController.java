...
public class ScheduleController {
	...
	@PostMapping
	public ResponseEntity<List<MonthScheduleResponse>> schedulesAdd(@Validated @RequestBody ScheduleAddRequest request,
			@PathVariable Long calendarId) throws SchedulerException {
		return ResponseEntity.ok(scheduleCommandService.addSchedules(request, calendarId).stream()
				.map(MonthScheduleResponse::from).toList());
	}
	...
	@GetMapping("/month")
	public ResponseEntity<List<MonthScheduleResponse>> schedulesInMonth(@PathVariable Long calendarId,
			@RequestParam Integer year, @RequestParam Integer month, @CurrentMember Member member) {
		return ResponseEntity.ok(scheduleQueryService.searchSchedulesInMonth(calendarId, year, month, member.getId()));
	}

	@GetMapping("/day")
	public ResponseEntity<List<DayScheduleResponse>> schedulesInDay(@PathVariable Long calendarId,
			@RequestParam LocalDate date, @CurrentMember Member member) {
		return ResponseEntity.ok(scheduleQueryService.searchSchedulesInDay(calendarId, date, member.getId()));
	}

	@GetMapping("/search")
	public ResponseEntity<List<DayScheduleResponse>> searchSchedules(@Size(min = 1) @RequestParam String query,
			@PathVariable Long calendarId, @CurrentMember Member member) {
		return ResponseEntity.ok(scheduleQueryService.searchSchedulesContainQuery(query, calendarId, member.getId()));
	}

	@PutMapping("/{scheduleId}")
	public ResponseEntity<Void> scheduleModify(@PathVariable Long scheduleId,
			@RequestBody ScheduleModifyRequest request, @CurrentMember Member member) throws SchedulerException {
	...
	public ResponseEntity<Void> scheduleRemoveById(@PathVariable Long scheduleId, @CurrentMember Member member)
	...
	public ResponseEntity<Void> schedulesRemoveInGroup(@PathVariable Long scheduleId, @CurrentMember Member member)
			throws SchedulerException {
	...
	public ResponseEntity<Void> schedulesRemoveInGroupAfter(@PathVariable Long scheduleId,
			@CurrentMember Member member) {
