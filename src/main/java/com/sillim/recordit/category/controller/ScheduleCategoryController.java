...
public class ScheduleCategoryController {
	...
	@GetMapping
	public ResponseEntity<List<ScheduleCategoryListResponse>> calendarCategoryList(@PathVariable Long calendarId,
			@CurrentMember Member member) {
		return ResponseEntity.ok(scheduleCategoryQueryService.searchScheduleCategories(calendarId, member.getId())
				.stream().map(ScheduleCategoryListResponse::of).toList());
	}

	@PostMapping
	public ResponseEntity<Void> addCalendarCategory(@PathVariable Long calendarId,
	...
	public ResponseEntity<Void> modifyCalendarCategory(@PathVariable Long calendarId, @PathVariable Long categoryId,
			@Valid @RequestBody ScheduleCategoryModifyRequest request, @CurrentMember Member member) {
		scheduleCategoryCommandService.modifyCategory(request, categoryId, calendarId, member.getId());
		return ResponseEntity.noContent().build();
	}

	@DeleteMapping("/{categoryId}")
	public ResponseEntity<Void> deleteCalendarCategory(@PathVariable Long calendarId, @PathVariable Long categoryId,
			@CurrentMember Member member) {
