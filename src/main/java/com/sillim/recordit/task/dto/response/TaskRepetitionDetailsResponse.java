...
@Builder
public record TaskRepetitionDetailsResponse(TaskRepetitionType repetitionType, Integer repetitionPeriod,
		LocalDate repetitionStartDate, LocalDate repetitionEndDate, Integer monthOfYear, Integer dayOfMonth,
		Integer weekNumber, Integer weekday, Integer weekdayBit) {

	public static TaskRepetitionDetailsResponse from(TaskRepetitionPattern repetitionPattern) {

		return TaskRepetitionDetailsResponse.builder().repetitionType(repetitionPattern.getRepetitionType())
				...
				.weekNumber(repetitionPattern.getWeekNumber().map(WeekNumber::getValue).orElse(null))
				.weekday(repetitionPattern.getWeekday().map(Weekday::getValue).orElse(null))
				.weekdayBit(repetitionPattern.getWeekdayBit().orElse(null)).build();
	}
}
